...
try {
	...
	if(isset($_POST['search'])) {
		...
		if(isset($_GET['page'])) {
			...
			$query = "SELECT DISTINCT p.*, s.*, u.user_id, u.user_name FROM product p, shop s, user u WHERE p.shop_id = s.shop_id AND s.user_id = u.user_id AND post_at = '$searchDate' AND product_status='active' LIMIT $page1, 4";
			...
			if ($count_row != 0) {
				...
				foreach($result as $row) {
					...
					if($location!="") {
						...
						echo "<td><a onClick=\"javascript: return confirm('Are you sure that you want to block this product?');\"
							href='blockProduct.php?productID=".$row['product_id']."' >
							<img src='/BuyMe/image/block.png' /></a></td>";
						echo "</tr>";
				...
				echo "<tr><td colspan='3'>
						<p></p>
						<label>No products are created on the selected date.</label>
						<p></p>
					  </td></tr>";
			}
			...
			$query1 = "SELECT DISTINCT p.*, s.*, u.user_id, u.user_name FROM product p, shop s, user u WHERE p.shop_id = s.shop_id AND s.user_id = u.user_id AND post_at = '$searchDate' AND product_status='active'";
			...
			for($page_next=1;$page_next<=$page;$page_next++) {
				echo "<li class='page-item'><a class='page-link' href='selling.php?page=$page_next'>$page_next</a></li>";
			}
			echo '</ul>';
			echo '</nav>';
		}		
		...
		if(isset($_GET['page'])) {
			...
			// Query the database and get the count
			$query = "SELECT DISTINCT p.*, s.*, u.user_id, u.user_name FROM product p, shop s, user u WHERE p.shop_id = s.shop_id AND s.user_id = u.user_id AND post_at = '$curr_date' AND product_status='active' LIMIT $page1, 4";
			$stmt = $conn->prepare($query);
			$stmt->execute();
			...
			if ($count_row != 0) {
				...
				echo "<tr><td colspan='3'>
						<p></p>
						<label>No products are created today.</label>
						<p></p>
					  </td></tr>";
			...
			$query1 = "SELECT DISTINCT p.*, s.*, u.user_id, u.user_name FROM product p, shop s, user u WHERE p.shop_id = s.shop_id AND s.user_id = u.user_id AND post_at = '$curr_date' AND product_status='active'";
			$stmt1 = $conn->prepare($query1);
			$stmt1->execute();
			$count_row1 = $stmt1->rowCount();
			$page = ceil($count_row1/4);
		...
		if(isset($_GET['page'])) {
			...
			// Query the database and get the count
			$query = "SELECT DISTINCT p.*, s.*, u.user_id, u.user_name FROM product p, shop s, user u WHERE p.shop_id = s.shop_id AND s.user_id = u.user_id AND product_status='active' LIMIT $page1, 4";
			$stmt = $conn->prepare($query);
			$stmt->execute();
			...
			$query1 = "SELECT DISTINCT p.*, s.*, u.user_id, u.user_name FROM product p, shop s, user u WHERE p.shop_id = s.shop_id AND s.user_id = u.user_id AND product_status='active'";
			$stmt1 = $conn->prepare($query1);
			$stmt1->execute();
			$count_row1 = $stmt1->rowCount();
			$page = ceil($count_row1/4);
